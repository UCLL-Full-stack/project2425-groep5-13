...
const getAllStudentAssociations = async (): Promise<StudentAssociation[]> => {
    try {
        ...
        return StudentAssociationPrisma.map((StudentAssociationPrisma) => StudentAssociation.from(StudentAssociationPrisma));
    } catch (error) {
        console.error(error);
        throw new Error('Database error. See server log for details.');
}}

const addUserToStudentAssociation = async ({user, kboNumber}: {user: UserInput, kboNumber: string}): Promise<StudentAssociation> => {
    try {
