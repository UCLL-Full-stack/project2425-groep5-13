...
const getUserByStudentNumber = async ({ studentNumber }: { studentNumber: string }): Promise<User> => {
...
const authenticate = async ({ studentNumber, password }: UserInput): Promise<AuthenticationResponse> => {
    ...
    return {
        token: generateJwtToken({ studentNumber, role: user.getRole() }),
