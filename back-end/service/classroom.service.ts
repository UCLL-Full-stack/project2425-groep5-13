...
const getClassroomByCampusAndClassroomNumber = async ({
    ...
    const classroom = await classroomDb.getClassroomByCampusAndClassroomNumber({
        campus,
        classroomNumber,
    });
    if (!classroom)
        throw new Error(`Classroom with ${campus} and ${classroomNumber} does not exist.`);
    return classroom;
};
...
const createClassroom = async ({ campus, classroomNumber }: ClassroomInput): Promise<Classroom> => {
    ...
    const existingClassroom = await classroomDb.getClassroomByCampusAndClassroomNumber({
        campus,
        classroomNumber,
    });
